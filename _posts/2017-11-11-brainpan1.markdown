...
That means that if we overwrite the address of a `jmp esp` instruction we can hop right into our shellcode! Use `CTRL-F`, search for the instruction and copy the address.
...
**Check out [NetSec article](https://netsec.ws/?p=337) for more TTYs if needed!** 
